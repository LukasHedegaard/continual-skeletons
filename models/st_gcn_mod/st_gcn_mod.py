...
class StGcnMod(
        ...
        self.layers = nn.ModuleDict(
            {
                ...
                "layer5": StGcnBlock(64, 128, A, stride=1),
                "layer6": StGcnBlock(128, 128, A),
                "layer7": StGcnBlock(128, 128, A),
                "layer8": StGcnBlock(128, 256, A, stride=1),
    ...
    def forward(self, x):
        ...
        # N*M,C,T,V
        c_new = x.size(1)
        x = x[:, :, 40:, :]  # CoStGcn has a initial delay of 40: account for it
...
class TemporalConvolution(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1):
        ...
        self.pad = 0  # Modified from int((kernel_size - 1) / 2)
