...
for subset in ["xview", "xsub"]:
    subprocess.call(
        [
            ...
            "--log_as",
            f"StGcn/eval_{DS_NAME}_{subset}_twostream",
            "--labels",
            str(DS_PATH / subset / "val_label.pkl"),
            "--pred1",
            str(ROOT_PATH / "preds" / f"stgcn_{DS_NAME}_{subset}_joint.npy"),
            "--pred2",
            str(ROOT_PATH / "preds" / f"stgcn_{DS_NAME}_{subset}_bone.npy"),
        ]
    )
